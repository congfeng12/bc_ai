<template>
  <div style="width: 100%;background-color: #F2F6FC;height: 100%">
    <div style="width: 100%;height: 100%;">
      <!-- 登录框 -->
      <div
        style="width: 800px;height: 400px;background-color: #FFFFFF;position: fixed;top: 200px;margin: 0 auto 0;left: 0;right: 0;border-radius: 15px;box-shadow: #EBEEF5 2px 2px 10px 2px;background-image:url(../../static/FormBackgroundImg.png)"
      >
        <!-- 表单 -->
        <div style="width: 370px;height: 400px;">
          <div style="width: 370px;height: 70px;position: relative;">
            <a :href="HomePageImgUrl">
              <img :src="FormPageLogo" style="height: 50px;margin: 20px 0px 0px 30px;" />
            </a>
            <font
              style="position: absolute;top: 35px;left: 100px;font-size: 1.2em;font-weight: bolder;"
            >蜂巢系统资源平台</font>
          </div>
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="20px">
            ...
            <el-form-item prop="password">
              <el-input
                ...
                type="password"
                @keyup.enter.native="platform_login"
                style="width: 330px;"
            ...
            <el-form-item>
              <el-button
                type="primary"
                @click="platform_login"
                style="width: 200px;margin:20px 0px 0px 65px;border-radius: 50px;"
              >
                <font style="letter-spacing: 0.8em;">登录</font>
              </el-button>
            </el-form-item>
          </el-form>
        </div>
      ...
      <div
        style="width: 360px;height: auto;margin: 0 auto 0;clear: both;position: fixed;top: 620px;left: 0;right: 0;text-align: center;"
      >
        <font
          style="color: #C0C4CC;font-size: 0.5em;"
        >{{Record_Number}} &nbsp&nbsp&nbsp {{Run_Time_Range}}&nbsp&nbsp{{Domain_Name}}</font>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    platform_login() {
      ...
      this.$Axios
        .post(
          this.$Global.Back_End_Service + this.$Global.login,
          this.$qs.stringify({
  ...
  mounted() {
    //页面加载后执行
    this.$Global.warning_notify(
      this,
      "通知",
      "为了能够提供更好的浏览体验请使用IE8以上的浏览器或者chrome浏览器等。我们将使用您的ip地址，用于保证账户的安全性。我们将不会在其他位置使用这些信息，您的这些信息将完全保密！",
      3000
    );
  }
};
