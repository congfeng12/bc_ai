...
export default {
  ...
  methods: {
    ...
    selectByCriteria() {
      ...
      this.$Axios
        .post(
          this.$Global.Back_End_Service + this.$Global.selectWorks,
          this.$qs.stringify({
            page: this.page,
            num: 20,
            search: this.search,
        ...
        .then(function(res) {
          if (res.data.RTCODE == "success") {
            ...
            for (var i = 0; i < res.data.RTDATA.data.length; ++i) {
              ...
              c_resource.place = res.data.RTDATA.data[i].place;
              c_resource.msg = res.data.RTDATA.data[i].content;
              c_resource.creatuser = res.data.RTDATA.data[i].createuser;
              c_resource.creatdate = res.data.RTDATA.data[i].createtime;
              c_resource.amount = res.data.RTDATA.data[i].need;
