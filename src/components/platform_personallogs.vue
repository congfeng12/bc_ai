    ...
    <div style="width: 1150px;height: 100%;margin: 0 auto 0;position: relative;">
       ...
       <el-card style="position: absolute;top: 20px;width: 100%;height: auto;margin-bottom: 50px;">
         <!-- 全局搜索 -->
          ...
          <el-button style="position: absolute;right: 20px;" type="primary" @click="selectUserOperationLogByCriteria" size="mini" round >搜索</el-button>
          <el-button style="position: absolute;right: 20px;top: 60px;" type="success" size="mini" round plain  @click="getOperationLogByParamsToExcel">导出EXCEL</el-button>
...
export default {
  ...
  methods: {  
    ...
    selectUserOperationLogByCriteria() {
        ...
        //请求公告信息
         this.$Axios.post(this.$Global.Back_End_Service+this.$Global.selectUserOperationLogByCriteria,this.$qs.stringify({
          search:this.search,
          ...
          if (res.data.RTCODE == 'success') {
            ...
            if (null != res.data.RTDATA.data) {
              ...
              if (0 < res.data.RTDATA.total && 0 == res.data.RTDATA.data.length) {
                that.page = 1;
                that.selectUserOperationLogByCriteria();
              }
    ...
    getOperationLogByParamsToExcel(){
      var that = this;
      this.$Axios.post(this.$Global.Back_End_Service+this.$Global.getOperationLogByParamsToExcel,this.$qs.stringify({
          search:this.search,
          timeaxisdate:this.v_timeaxisdate,
          logstype:this.logstype,
          num:this.total,
       ...
         .then(function (res) {
          ...
          [res.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8' })
                    ...
                    aEle.href = href;
                    aEle.download = 'OperationLog_'+Math.floor(Math.random()*10000000 + 1)+'.xlsx';  // 下载后文件名
...
      })
    ...
    handleCurrentChange(val){
      this.page = val;
      this.selectUserOperationLogByCriteria();
    }
    },
  created(){
    //页面加载时执行
